	...
	<div id="boxes">
		<div style="top: 199.5px; left: 551.5px; display: none;" id="dialog"
			...
			<div id="lorem">
				<table class="table table-bordered">
					...
					<tbody>
						<c:forEach items="${message_list}" var="m">
							<c:if test="${login_users.id eq m.send_id}">
								<tr>
									<td id="td">${m.send_id}</td>
									<td id="td">${m.m_contents}</td>
									<td id="td"><a
										href="m_write_view?receive_id=${m.receive_id}">${m.receive_id}</a></td>
				...
				<div id="popupfoot">
					<button type="button" style="padding-top: 15px;"
						onclick="location.href='main'" class="close agree">닫기</button>
				</div>
			</div>
		</div>
		<div
