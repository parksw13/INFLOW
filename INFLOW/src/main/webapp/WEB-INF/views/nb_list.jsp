				...
				<table class="table table-bordered" id="lorem">
					...
					<tbody>
						<c:forEach var="nbbean" items="${board_list}">
							<tr>
								<td id="td">${nbbean.nb_num}</td>
								<td id="td">${nbbean.nb_writer}</td>
								<td id="td"><c:forEach begin="1" end="${bbean.bId}">-</c:forEach>
									<a href="nb_content_view?nb_num=${nbbean.nb_num}">${nbbean.nb_title}</a></td>
								<td id="td"><fmt:parseDate var="date"
										value="${nbbean.nb_date}" pattern="yyyy-MM-dd" /> <fmt:formatDate
										value="${date}" pattern="yyyy년 MM월 dd일" /></td>
								<td id="td">${nbbean.nb_count}</td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
				<c:if test="${login_users.grade eq 'A'}">
					<button type="button" onclick="location.href='nb_write_view'">글쓰기</button>
					<div id="nav">
